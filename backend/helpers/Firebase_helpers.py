...
async def get_current_user(
    ...
    try:
        ...
        firebase_user = FirebaseUser(
            uid=uid, email=email, roles=roles, name=name, token=token
        )
        ...
        raise HTTPException(
            status_code=401, detail="Invalid Firebase ID token. Please sign in again."
        )
    except Exception as e:
        raise HTTPException(
            status_code=500,
            detail=f"Unexpected error validating Firebase ID token: {str(e)}",
        )
...
def role_based_access(required_roles: List[str]) -> Callable:
    ...
    async def check_role(
        request: Request, current_user: FirebaseUser = Depends(get_current_user)
    ) -> None:
        """
        ...
        try:
            ...
            raise HTTPException(
                status_code=500, detail=f"Error retrieving user claims: {str(e)}"
            )
